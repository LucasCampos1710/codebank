...
func (t *TransactionRepositoryDb) GetCreditCard(creditCard domain.CreditCard) (domain.CreditCard, error) {
	...
	if err = stmt.QueryRow(creditCard.Number).Scan(&c, &c.Balance, &c.Limit); err != nil {
		return c, errors.New("credit card doest not exist")
	}
	return c, nil
}
...
func (t *TransactionRepositoryDb) SaveTransaction(transaction domain.Transaction, creditCard domain.CreditCard) error {
	 stmt, err := t.db.Prepare(` insert into transactions(id, credit_car_id, amount, status, description, store, created_at)
...
func (t *TransactionRepositoryDb) CreateCreditCard(creditCard domain.CreditCard) error {
	stmt, err := t.db.Prepare(`insert into credit_card(id, name, number, expiration_month, expiration_year, CVV, balance, balance_limit)
